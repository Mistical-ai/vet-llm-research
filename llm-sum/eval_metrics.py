"""
llm-sum/eval_metrics.py — Lightweight automatic evaluation metrics
===================================================================

WHY THIS MODULE EXISTS
----------------------
MedHELM reports automatic metrics alongside jury scores. This module implements
small deterministic metrics that run locally, require no GPU, and make no API
calls. They are secondary evidence only; the blind veterinary jury remains the
primary validity signal.
"""
...
# A deliberately small tokenizer keeps these metrics dependency-free. These
# metrics are not meant to replace a clinical judge; they are cheap signals that
# can be recomputed in any environment.
TOKEN_RE = re.compile(r"[A-Za-z0-9]+")

# Section coverage is a heuristic checklist for the summary text. The regexes
# are intentionally broad because veterinary papers phrase the same section in
# many ways, and this score is only secondary evidence.
SECTION_PATTERNS = {
    ...
    "species_sample": re.compile(r"\b(dog|dogs|canine|cat|cats|feline|horse|horses|equine|cow|cattle|bovine|n\s*=\s*\d+|\d+\s+(dogs|cats|horses|cattle|animals|patients))\b", re.I),
    "results": re.compile(r"\b(result|finding|significant|p\s*[<=>]|increased|decreased|associated)\b", re.I),
    "clinical_significance": re.compile(r"\b(clinical|clinician|practice|treatment|diagnosis|prognosis|recommend)\b", re.I),
...
def tokenize(text: str) -> list[str]:
    ...
    A simple regex tokenizer is chosen over a model tokenizer because these
    metrics are coarse sanity checks and must run in every test environment.
    """
...
def rouge_recall(reference_text: str, candidate_summary: str, n: int = 1) -> float:
    ...
    Recall is used because the research question asks how much source content
    the summary captures. Precision-like measures can be misleading when a
    short summary correctly omits irrelevant article details.
    """
...
def extractive_coverage(reference_text: str, candidate_summary: str) -> float:
    ...
    This is not a hallucination detector. It is a cheap signal for whether a
    summary is highly abstractive or mostly copied from the article.
    """
...
def calculate_automatic_metrics(
    ...
    *,
    ...
    When a gold abstract or reference summary is available, callers can pass it
    as reference_summary for ROUGE. Otherwise ROUGE is computed against the
    cleaned article text as a coarse recall signal and should be interpreted
    cautiously.
    """
